...
int bitio_flush(struct bitio* b){//If the mode isn't write mode, error.
	...
	for(;;){
		...
		left -= x;
		start += x;
		if(left == 0){//I write a char at time. Each time I decrease left from x (the number of char to write) and increase start to x (the offset).
			break;
		}
	}
	char* dst;
...
int bitio_read(struct bitio* b){
	...
	if(b->bitio_rp == b->bitio_wp){//If the read and write pointer are in the same position they assume position 0
		...
		for(;;){
			x = read(b->bitio_fd, b->buf, sizeof(b->buf));//I read until i fill the buffer. 
			if(x < 0)
				return -1;
			else if(x == 0){
				continue;
